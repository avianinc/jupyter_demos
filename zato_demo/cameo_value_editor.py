...
class GetUserDetails(Service):
    ...
    class SimpleIO:
        input_required = 'value', 'elementId'
    ...
    def handle(self):
        ...
        id = self.request.input.elementId
        dataValue = {"kerml:esiData":{"value":"0.0"}}
        dataValue["kerml:esiData"]["value"] = value

        # A command
        command = f'curl -k -X PATCH "https://18.205.77.131:8111/osmc/resources/272e28f2-45b7-45cb-a016-800ba747e716/elements/{id}" -H "accept: application/ld+json" -H "authorization: Basic amRlaGFydDpqa2QyMjE0" -H "Content-Type: application/ld+json" -d "{dataValue}"'
